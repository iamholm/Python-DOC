...
def print_and_export_to_pdf(docx_path, pdf_path):
...
def generate_documents(kusp, date_reg, fio, dob, address, selected_emp_key, employee_dict):
    ...
    # Печать + PDF
    print_and_export_to_pdf(postanov_filled, postanov_pdf_out)
    print_and_export_to_pdf(sopr_filled,     sopr_pdf_out)
...
def main():
    ...
    root = tk.Tk()
    root.title("Документы по трупу 15 отдела полиции")
    ...
    def on_button_click():
        ...
        try:
            ...
            messagebox.showinfo(
                "Успех",
                "Документы созданы и отправлены на печать!"
            )
    ...
    button_generate = tk.Button(root, text="Печать и сохранить в PDF", command=on_button_click)
