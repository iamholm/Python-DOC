...
#########################
# (Глобальная переменная) Word-приложение
#########################
word_app = None  # Будем хранить экземпляр Word, чтобы один раз запустить и один раз закрыть
...
def export_to_pdf_and_open(docx_path, pdf_path):
    """
    Открывает docx в уже запущенном Word (глобальная переменная word_app),
    сохраняет в PDF, закрывает документ (но не Word),
    затем открывает PDF.
    """
    global word_app
    if word_app is None:
        raise RuntimeError("Word не инициализирован. Необходимо сначала запустить Word-приложение.")

    # Открываем документ
    doc = word_app.Documents.Open(docx_path)
    # Экспорт в PDF
    doc.ExportAsFixedFormat(pdf_path, 17, OpenAfterExport=False)
    doc.Close(False)  # Закрываем документ, но не Word

    # Открываем PDF в системе
...
def main():
    ...
    # Не запускаем Word сразу – сначала показываем окно!
    # Word запустим отложенно через root.after()

    employees_data = load_employees()
    config_data = load_config()

    employee_dict = {
        f"{emp['Фамилия']} {emp['Инициалы']}": emp
        for emp in employees_data["employees"]
    }

    root = tk.Tk()
    root.title("Труп Документы     15 отдел полиции     v. 1.0.1")

    # Если есть icon.ico рядом с программой, установить иконку
    ...
    def start_word():
        """Функция для отложенного запуска Word после появления основного окна."""
        global word_app
        try:
            word_app = comtypes.client.CreateObject("Word.Application")
            word_app.Visible = False  # или True, если нужно видеть окно Word
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось запустить Word: {e}")

    # Запускаем Word через 500 мс после старта (0.5 секунды)
    root.after(500, start_word)
    ...
    def on_close():
        # Сохраняем выбранного сотрудника в config
        cfg = {"selected_employee": combo_employee.get()}
        save_config(cfg)

        # Закрываем Word-приложение, если оно было запущено
        global word_app
